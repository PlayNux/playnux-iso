...
APP_BASE = \
	applications/nux-chroot-gui.desktop \
...
edit = sed -e "s|@datadir[@]|$(DESTDIR)$(PREFIX)/share/nux-tools|g" \
	-e "s|@sysconfdir[@]|$(DESTDIR)$(SYSCONFDIR)/nux-tools|g" \
	-e "s|@libdir[@]|$(DESTDIR)$(PREFIX)/lib/nux-tools|g" \
	-e "s|@version@|${Version}|"
...
install_base:
	install -dm0755 $(DESTDIR)$(SYSCONFDIR)/nux-tools
	install -m0644 ${SYSCONF} $(DESTDIR)$(SYSCONFDIR)/nux-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/bin
	install -m0755 ${BIN_BASE} $(DESTDIR)$(PREFIX)/bin

	install -dm0755 $(DESTDIR)$(PREFIX)/lib/nux-tools
	install -m0644 ${LIBS_BASE} $(DESTDIR)$(PREFIX)/lib/nux-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/share/nux-tools
	install -m0644 ${SHARED_BASE} $(DESTDIR)$(PREFIX)/share/nux-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/share/applications
	install -m0644 ${APP_BASE} $(DESTDIR)$(PREFIX)/share/applications

install_pkg:
	install -dm0755 $(DESTDIR)$(SYSCONFDIR)/nux-tools/pkg.list.d
	install -m0644 ${LIST_PKG} $(DESTDIR)$(SYSCONFDIR)/nux-tools/pkg.list.d

	install -dm0755 $(DESTDIR)$(SYSCONFDIR)/nux-tools/make.conf.d
	install -m0644 ${ARCH_CONF} $(DESTDIR)$(SYSCONFDIR)/nux-tools/make.conf.d
	...
	install -dm0755 $(DESTDIR)$(PREFIX)/lib/nux-tools
	install -m0644 ${LIBS_PKG} $(DESTDIR)$(PREFIX)/lib/nux-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/share/nux-tools
	install -m0644 ${SHARED_PKG} $(DESTDIR)$(PREFIX)/share/nux-tools
...
install_iso:
	install -dm0755 $(DESTDIR)$(SYSCONFDIR)/nux-tools/iso.list.d
	install -m0644 ${LIST_ISO} $(DESTDIR)$(SYSCONFDIR)/nux-tools/iso.list.d

	install -dm0755 $(DESTDIR)$(PREFIX)/bin
	install -m0755 ${BIN_ISO} $(DESTDIR)$(PREFIX)/bin

	install -dm0755 $(DESTDIR)$(PREFIX)/lib/nux-tools
	install -m0644 ${LIBS_ISO} $(DESTDIR)$(PREFIX)/lib/nux-tools
	...
	install -dm0755 $(DESTDIR)$(PREFIX)/share/nux-tools
	install -m0644 ${SHARED_ISO} $(DESTDIR)$(PREFIX)/share/nux-tools
	...
	gzip -c man/nux-tools.conf.5 > $(DESTDIR)$(PREFIX)/share/man/man5/nux-tools.conf.5.gz
...
install_yaml:
	...
	install -dm0755 $(DESTDIR)$(PREFIX)/lib/nux-tools
	install -m0644 ${LIBS_YAML} $(DESTDIR)$(PREFIX)/lib/nux-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/share/nux-tools
	install -m0644 ${SHARED_YAML} $(DESTDIR)$(PREFIX)/share/nux-tools

	install -dm0755 $(DESTDIR)$(PREFIX)/share/man/man1
	gzip -c man/check-yaml.1 > $(DESTDIR)$(PREFIX)/share/man/man1/check-yaml.1.gz

uninstall_base:
	for f in ${SYSCONF}; do rm -f $(DESTDIR)$(SYSCONFDIR)/nux-tools/$$f; done
	for f in ${BIN_BASE}; do rm -f $(DESTDIR)$(PREFIX)/bin/$$f; done
	for f in ${SHARED_BASE}; do rm -f $(DESTDIR)$(PREFIX)/share/nux-tools/$$f; done
	for f in ${LIBS_BASE}; do rm -f $(DESTDIR)$(PREFIX)/lib/nux-tools/$$f; done

uninstall_pkg:
	for f in ${LIST_PKG}; do rm -f $(DESTDIR)$(SYSCONFDIR)/nux-tools/pkg.list.d/$$f; done
	for f in ${ARCH_CONF}; do rm -f $(DESTDIR)$(SYSCONFDIR)/nux-tools/make.conf.d/$$f; done
	for f in ${BIN_PKG}; do rm -f $(DESTDIR)$(PREFIX)/bin/$$f; done
	rm -f $(DESTDIR)$(PREFIX)/bin/find-libprovides
	for f in ${SHARED_PKG}; do rm -f $(DESTDIR)$(PREFIX)/share/nux-tools/$$f; done
	for f in ${LIBS_PKG}; do rm -f $(DESTDIR)$(PREFIX)/lib/nux-tools/$$f; done
	rm -f $(DESTDIR)$(PREFIX)/share/man/man1/buildpkg.1.gz
	rm -f $(DESTDIR)$(PREFIX)/share/man/man1/buildtree.1.gz

uninstall_iso:
	for f in ${LIST_ISO}; do rm -f $(DESTDIR)$(SYSCONFDIR)/nux-tools/iso.list.d/$$f; done
	for f in ${BIN_ISO}; do rm -f $(DESTDIR)$(PREFIX)/bin/$$f; done
	for f in ${SHARED_ISO}; do rm -f $(DESTDIR)$(PREFIX)/share/nux-tools/$$f; done

	for f in ${LIBS_ISO}; do rm -f $(DESTDIR)$(PREFIX)/lib/nux-tools/$$f; done
	...
	rm -f $(DESTDIR)$(PREFIX)/share/man/man5/nux-tools.conf.5.gz
	rm -f $(DESTDIR)$(PREFIX)/share/man/man5/profile.conf.5.gz

uninstall_yaml:
	for f in ${BIN_YAML}; do rm -f $(DESTDIR)$(PREFIX)/bin/$$f; done
	for f in ${LIBS_YAML}; do rm -f $(DESTDIR)$(PREFIX)/lib/nux-tools/$$f; done
	for f in ${SHARED_YAML}; do rm -f $(DESTDIR)$(PREFIX)/share/nux-tools/$$f; done
...
dist:
	git archive --format=tar --prefix=nux-tools-$(Version)/ $(Version) | gzip -9 > nux-tools-$(Version).tar.gz
	gpg --detach-sign --use-agent nux-tools-$(Version).tar.gz
