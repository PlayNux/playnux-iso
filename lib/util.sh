...
init_common(){
    ...
    [[ -z ${cache_dir} ]] && cache_dir='/var/cache/nux-tools/nux-builds'

    [[ -z ${chroots_dir} ]] && chroots_dir='/var/cache/nux-tools/nux-chroots'

    [[ -z ${log_dir} ]] && log_dir='/var/cache/nux-tools/nux-logs'

    [[ -z ${build_mirror} ]] && build_mirror='http://mirrors.kernel.org'

    [[ -z ${tmp_dir} ]] && tmp_dir='/tmp/nux-tools'
}
...
init_buildtree(){
    ...
    [[ -z ${host_tree} ]] && host_tree='https://gitlab.com/nux-linux'
...
init_buildiso(){
    ...
    [[ -z ${dist_branding} ]] && dist_branding="nux"
...
load_profile_config(){
    ...
    [[ -z ${hostname} ]] && hostname="nux"

    [[ -z ${username} ]] && username="nux"

    [[ -z ${use_dracut} ]] && use_dracut="true"

    [[ -z ${plymouth_theme} ]] && plymouth_theme="nux"

    [[ -z ${password} ]] && password="nux"
    ...
    if [[ -z ${enable_systemd_live[@]} ]]; then
        enable_systemd_live=('nux-live' 'mhwd-live' 'pacman-init' 'mirrors-live')
    fi
    ...
    netgroups="https://gitlab.com/nux-linux/packages/pkgbuilds/nux-pkgbuilds/-/raw/master/pkgbuilds/calamares-netinstall-settings/netinstall-software.yaml"
...
load_user_info(){
    ...
    USERCONFDIR="$USER_HOME/.config/nux-tools"
...
show_config(){
    if [[ -f ${USERCONFDIR}/nux-tools.conf ]]; then
        msg2 "config: %s" "~/.config/nux-tools/nux-tools.conf"
    else
...
init_profiles() {	
	...
	git clone -q --depth 1 -b ${branch} https://gitlab.com/nux-linux/tools/iso-profiles.git ${_workdir}/iso-profiles/
	
	#Check if git clone is done
	if [[ -d ${_workdir}/iso-profiles/nux ]] || [[ -d ${_workdir}/iso-profiles/community ]]; then
	...
	else msg2 "Impossible to initialize iso-profiles, please check internet connection or browse at 'https://gitlab.com/nux-linux/tools/iso-profiles'"
	exit 1
	fi
}
