...
configure_mhwd_drivers(){
    ...
    if ! lslist_contains_package "$packagelist" nux-virtualmachine-guest-config; then
...
style:
   ...
   SidebarBackgroundCurrent: "\"${sidebarbackgroundcurrent}"\"" > $1/usr/share/calamares/branding/nux/branding.desc
}
...
configure_system(){
    ...
    echo 'File created by nux-tools. See systemd-update-done.service(8).' \
...
clean_up_image(){
	...
	rm -f "$1"/var/lib/nux/partial_upgrade
    rm -rf "$1"/var/lib/nux/tmp/
}
