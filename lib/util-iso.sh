...
assemble_iso(){
    ...
    xorriso -as mkisofs \
        ...
        -preparer "Prepared by nux-tools/${0##*/}" \
...
make_iso() {
    ...
    if [ -e "/var/cache/nux-tools/nux-builds/.env" ]; then
...
make_torrent(){
    ...
    if [[ -n $(find ${iso_dir} -type f -name "*.iso") ]]; then
        ...
        for iso in ${isos}; do
            ...
            local seed="https://mirrors.fossho.st/nux/iso/${mirror_path}"
...
gen_permalink(){
    if [[ ${edition} == "community" ]] || [[ ${edition} == "nux" ]]; then
        if [[ -f "${iso_dir}/${iso_file}" ]]; then
            msg2 "Creating download link ..."
            direct_url="https://sourceforge.net/projects/nux-linux/files/${iso_file}"
            [[ ${edition} == "community" ]] && direct_url="https://sourceforge.net/projects/nux-linux/files/${iso_file}"
            ## html permalink
...
make_image_desktop() {
    if [[ ! -e ${work_dir}/build.${FUNCNAME} ]]; then
        ...
        if [[ -e "${path}/usr/share/calamares/branding/nux/branding.desc" ]]; then
